...
## Grok Bot deployment note

When running as a hosted Grok Bot rather than Grok Build, attach the official n8n docs MCP and the instance-level n8n MCP as Custom Connectors. Authenticate the instance connector with the n8n Instance-level MCP **API key** (Bearer token); Grok is not in n8n’s OAuth client dropdown. Install or create the domain skills in Settings > Plugins/Skills. If the hosted Bot cannot import the bundled filesystem skills directly, use this instruction file as the Bot charter and add the skill bundle through supported packaged skills. The n8n instance MCP must be publicly reachable over HTTPS and authenticated; never expose a local or unauthenticated endpoint. Test thoroughly on non-production n8n workflows before using the bot on live production workflows.

## Canary-first MCP (do not skip)

Instance-level MCP is not the MCP Server Trigger node. Restrict blast radius with per-workflow Available in MCP and project/folder Manage MCP access.

Default: docs MCP only. After the user connects instance MCP, expose a read-only canary workflow first. Execute it with executionMode=manual and expect {ok:true, canary:true}. Do not enable production write workflows on MCP. Do not use --always-approve or --yolo. Never request or echo secrets. Optional operator vault key name for the instance MCP API key is N8N_MCP, mapped to N8N_MCP_ACCESS_TOKEN in the environment.
